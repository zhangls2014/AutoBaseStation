...
import com.zhangls.auto.camera.CompareSizesByArea
import com.zhangls.auto.camera.ImageSaver
...
class CameraActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        /**
         * Camera state: PictureModel was taken.
         */
        ...
        fun activityStart(activity: Context) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 设置顶部导航栏
        toolbar.title = getString(R.string.titleActivityCamera)
        setSupportActionBar(toolbar)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)

        createTime = System.currentTimeMillis()
        file = File(getExternalFilesDir(Environment.DIRECTORY_PICTURES),
                "Auto_$createTime.jpg")
    }
     ...
     * 保存数据
     *
     * @param createTime 拍照时间
     * @param path 图片路径
     */
    private fun saveData(createTime: Long, path: String) {
        doAsync {
            if (database == null) database = AbstractDatabase.get(this@CameraActivity)

            val model = PictureModel(createTime, false, path)
            database!!.pictureDao().insertPicture(model)

            uiThread {
                toast(R.string.toast_picture_capture_success)
                finish()
            }
        }
    }
    ...
    private fun createCameraPreviewSession() {
        try {
            ...
            // 2.5s 后拍照，等待自动对焦（这种处理方式不好，应该监听对焦事件，对焦完成后立即拍照，然而....）
            Handler().postDelayed({ lockFocus() }, 2500)
        } catch (e: CameraAccessException) {
            Log.e(TAG, e.toString())
        }
        ...
        try {
            ...
            val captureCallback = object : CameraCaptureSession.CaptureCallback() {
                ...
                override fun onCaptureCompleted(session: CameraCaptureSession,
                                                request: CaptureRequest,
                                                result: TotalCaptureResult) {
                    saveData(createTime, file.toString())
